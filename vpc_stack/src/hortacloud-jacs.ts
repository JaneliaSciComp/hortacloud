...
import {
  getHortaServicesConfig,
  HortaCloudServicesConfig,
} from "./hortacloud-services-config";
...
export class HortaCloudJACS extends Construct {
  ...
  constructor(scope: Construct, id: string, hortaVpc: IVpc) {
    super(scope, id);
      ...
      blockDevices: [
        {
          ...
          volume: ec2.BlockDeviceVolume.ebs(
            hortaConfig.hortaSystemVolumeSizeGB
          ),
        },
        {
          deviceName: "/dev/xvdb",
    ...
    const backupBucketName = hortaConfig.hortaBackupBucket
      ? hortaConfig.hortaBackupBucket
      : "";
    const restoreBucketName = hortaConfig.hortaRestoreBucket
      ? hortaConfig.hortaRestoreBucket
      : "";
    ...
    const externalDataBuckets = [
      ...new Set(externalDataBucketsCandidates.filter((s) => s)),
    ];
    ...
    const dataBackupFolder = hortaConfig.hortaBackupFolder
      ? hortaConfig.hortaBackupFolder
      : "/hortacloud/backups";
    const jacsGitBranchArgs: string[] = hortaConfig.jacsGitBranch
      ? ["--jacs-git-branch", hortaConfig.jacsGitBranch]
      : [];
    const backupArgs: string[] = backupBucketName
      ...
      : ["--no-backup"];
    const dataRestoreFolder =
      hortaConfig.hortaRestoreBucket && hortaConfig.hortaRestoreFolder
        ? hortaConfig.hortaRestoreFolder
        : "";
    const restoreArgs: string[] = dataRestoreFolder
      ? ["--restore", restoreBucketName, dataRestoreFolder]
      : ["--no-restore"];
    const restoreWorkersArgs: string[] =
      hortaConfig.hortaRestoreWorkers > 0
        ? [
            "--restore-workers",
            hortaConfig.hortaRestoreWorkers as unknown as string,
          ]
        : [];
    const restoreParallelCollectionArgs: string[] =
      hortaConfig.hortaRestoreParallelCollections > 0
        ? [
            "--restore-workers",
            hortaConfig.hortaRestoreParallelCollections as unknown as string,
          ]
        : [];
    const systemLogsBackupFolder = hortaConfig.hortaSystemLogsBackupFolder
      ? hortaConfig.hortaSystemLogsBackupFolder
      : "hortacloud/systemlogs";
    const systemLogsBackupArgs: string[] = backupBucketName
      ? [backupBucketName, systemLogsBackupFolder]
      : [];
    const mailServerArgs: string[] = hortaConfig.mailServer
      ? ["--mail-server", hortaConfig.mailServer]
      : [];
    const mailUserArgs: string[] = hortaConfig.mailUser
      ? ["--mail-user", hortaConfig.mailUser]
      : [];
    const mailPasswordArgs: string[] = hortaConfig.mailPassword
      ? ["--mail-password", hortaConfig.mailPassword]
      : [];
    const mailSenderArgs: string[] = hortaConfig.mailSender
      ? ["--mail-sender", hortaConfig.mailSender]
      : [];
    const mailReceiverArgs: string[] = hortaConfig.mailReceiver
      ? ["--mail-receiver", hortaConfig.mailReceiver]
      : [];
    const workstationCacheDirArgs: string[] = hortaConfig.workstationCacheDir
      ? ["--workstation_cache_dir", hortaConfig.workstationCacheDir]
      : [];
...
function createJacsMachineImage(
  cfg: HortaCloudServicesConfig
): HortaCloudMachine {
