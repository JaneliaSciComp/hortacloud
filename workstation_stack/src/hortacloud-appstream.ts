...
} from "aws-cdk-lib/aws-appstream";
import {
  createResourceId,
  VpcInstanceProps,
...
export class HortacloudAppstream extends Construct {
  constructor(scope: Construct, id: string, vpcProps: VpcInstanceProps) {
    ...
    // create the fleet
    const fleetInstanceName = createResourceId(
      hortaAppstreamConfig,
      "workstation-fleet"
    );
    const imageName = createResourceId(
      hortaAppstreamConfig,
      "HortaCloudWorkstation"
    );
    const fleetInstance = new CfnFleet(this, "HortaCloudFleet", {
      ...
      idleDisconnectTimeoutInSeconds:
        hortaAppstreamConfig.sessionIdleDisconnectInMin * 60,
    });
    ...
    const storageConnectors = [
      ...
      getStorageConnector(
        "GOOGLE_DRIVE",
        false,
        hortaAppstreamConfig.googleDomains
      ),
      getStorageConnector(
        "ONE_DRIVE",
        false,
        hortaAppstreamConfig.oneDriveDomains
      ),
    ].filter((s) => s.connectorType); // filter out the storage connectors without a valid connectorType

    // create the stack
    const stackInstanceName = createResourceId(
      hortaAppstreamConfig,
      "workstation-stack"
    );
    ...
    const stackFleetAssoc = new CfnStackFleetAssociation(
      this,
      "HortaCloudFleetStackAssoc",
      {
