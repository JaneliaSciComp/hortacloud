...
exports.handler = async (event, context, callback) => {
  if (!event.requestContext.authorizer) {
    //checks to see if Cognito Authorization has been configured
    errorResponse(
      "Authorization has not been configured, please configure an authorizer in API Gateway",
...
async function createas2streamingurl(params, awsRequestId, callback) {
  ...
  try {
    ...
    errorResponse(
      "Error creating AS2 streaming URL: " + JSON.stringify(err),
      awsRequestId,
      callback
    );
  }
